...
unsigned char write_chunk_riff(struct chunk_riff* chunk_riff, FILE* output_file) {
  ...
  size_t offset = 0;
  memcpy(output_buffer + offset, chunk_riff->ck_id, CHUNK_RIFF_CK_ID_LENGTH);
  offset += CHUNK_RIFF_CK_ID_LENGTH;
  memcpy(output_buffer + offset, chunk_riff->cksize, CHUNK_RIFF_CKSIZE_LENGTH);
  offset += CHUNK_RIFF_CKSIZE_LENGTH;
  memcpy(output_buffer + offset, chunk_riff->wave_id, CHUNK_RIFF_WAVE_ID_LENGTH);
